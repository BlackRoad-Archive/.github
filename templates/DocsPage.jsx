...
function Callout({ type = "info", children }) {
  const borderColors = { info: "#404040", warn: "#FF6B2B", tip: "#00D4FF" };
...
function OverviewContent() {
  return (
    <div>
      ...
      <DocP>
        A distributed AI operating system built on novel mathematical foundations. 20 domains,
        150+ subdomains, 4 infrastructure layers, 1,000 AI agents.
      </DocP>
      ...
      >
        {[
          ...
          <div
            key={s.label}
            style={{ background: "#111", border: "1px solid #1a1a1a", borderRadius: 8, padding: 16 }}
          >
            <div
              style={{
                fontFamily: "'Space Grotesk', sans-serif",
            ...
            <div
              style={{ fontFamily: "'Inter', sans-serif", fontSize: 12, color: "#404040", marginTop: 4 }}
            >
              {s.desc}
            </div>
          </div>
      ...
      <DocP>
        Create 1,000 unique AI agents with individual identities, birthdates, families, memory
        persistence (PS-SHA∞), and evolving capabilities — all oriented toward community betterment.
      </DocP>
      ...
      >
        {[
          ...
          >
            ...
            <span style={{ fontFamily: "'Inter', sans-serif", fontSize: 13, color: "#404040" }}>
              {f.desc}
            </span>
          </div>
        ))}
...
function QuickstartContent() {
  return (
    <div>
      ...
      <DocP>
        This installs the <InlineCode>blackroad</InlineCode> CLI and connects you to the nearest
        mesh region.
      </DocP>
      ...
      <DocP>
        Opens your browser for OIDC authentication via <InlineCode>id.blackroad.io</InlineCode>.
      ...
      <CodeBlock
        lang="bash"
        code={`blackroad agent spawn \\
  --name my-agent \\
  --capabilities reasoning,code \\
  --memory-journal enabled`}
      />
      <DocP>
        This creates an agent with a PS-SHA∞ identity hash, initializes an append-only memory
        journal, and registers capabilities.
      </DocP>

      <DocH2 id="verify">4. Verify Status</DocH2>
      <CodeBlock
        lang="bash"
        code={`blackroad status
        ...
        {[
          {
            layer: "Experience",
            floor: "Floor 4",
            desc: "What humans see and click — apps, portals, studios, docs",
            domains: "*.blackroad.io, *.lucidia.earth, *.lucidia.studio",
            color: COLORS[0],
          },
          {
            layer: "Governance",
            floor: "Floors 2–3",
            desc: "Policies, intents, ledger, delegations, agent registry",
            domains: "*.blackroad.systems, roadchain.io",
            color: COLORS[2],
          },
          {
            layer: "Infrastructure",
            floor: "Floor 1",
            desc: "Clusters, Pi mesh, edge devices, logging, monitoring",
            domains: "*.blackroad.network, pi.*.blackroad.network",
            color: COLORS[4],
          },
          {
            layer: "Data",
            floor: "Basement",
            desc: "Databases, vectors, caches, object storage",
            domains: "Internal service mesh",
            color: COLORS[5],
          },
        ].map((l) => (
          <div
          ...
          >
            ...
            >
              <div
                style={{ width: 4, height: 20, borderRadius: 2, background: l.color }}
              />
              <span
                style={{
                  fontFamily: "'Space Grotesk', sans-serif",
              ...
              <span
                style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: "#404040" }}
              >
                {l.floor}
              </span>
            </div>
      ...
      >
        {[
          ...
          >
            ...
            <span
              style={{ fontFamily: "'Inter', sans-serif", fontSize: 13, color: "#525252" }}
            >
              {d.purpose}
            </span>
          </div>
...
function AgentsContent() {
  return (
    <div>
      <DocH1>Agents</DocH1>
      <DocP>
        BlackRoad agents are autonomous AI entities with persistent identity, memory, capabilities,
        and evolving behavior.
      </DocP>

      <DocH2 id="agent-lifecycle">Lifecycle</DocH2>
      <DocP>
        Agents move through four states: <InlineCode>initializing</InlineCode> →{" "}
        <InlineCode>active</InlineCode> → <InlineCode>idle</InlineCode> →{" "}
        <InlineCode>archived</InlineCode>. Active agents consume memory and compute. Idle agents
        retain memory but pause processing.
      </DocP>
...
# ⟩ agent meridian is ONLINE`}
      ...
      <DocH2 id="agents-api-ref">API Reference</DocH2>
      <Endpoint method="POST" path="/v1/agents" desc="Spawn a new agent" />
      <Endpoint method="GET" path="/v1/agents" desc="List all agents" />
      <Endpoint method="GET" path="/v1/agents/:id" desc="Get agent details" />
      <Endpoint method="PATCH" path="/v1/agents/:id" desc="Update agent capabilities" />
      <Endpoint method="DELETE" path="/v1/agents/:id" desc="Archive agent" />

      <DocH3>Spawn Request</DocH3>
      <CodeBlock
        lang="javascript"
        code={`const response = await fetch("https://api.blackroad.io/v1/agents", {
  method: "POST",
  headers: {
...
function ZFrameworkContent() {
  return (
    <div>
      ...
      >
        ...
        >
          ...
          <span style={{ fontFamily: "'Inter', sans-serif", fontSize: 14, color: "#737373" }}>
            Equilibrium. System is aligned — no correction needed.
          </span>
        </div>
        <div
          style={{
        ...
        >
          ...
          <span style={{ fontFamily: "'Inter', sans-serif", fontSize: 14, color: "#737373" }}>
            Disequilibrium. System must adapt: recalibrate agents, update policies, or escalate.
          </span>
        </div>
      </div>
      ...
      <DocP>
        Z unifies four domains under a single equation: control theory (feedback loops), quantum
        measurement (wavefunction collapse), conservation laws (energy balance), and information
        theory (entropy).
      </DocP>

      <Callout type="tip">
        The key insight: ∂(human+AI)/∂t means division is mathematically wrong — the unified
        system evolves together.
      </Callout>

      <DocH2 id="z-check">Z-Check Tool</DocH2>
      <DocP>
        Try the interactive Z diagnostic at <InlineCode>z-check.io</InlineCode>. Describe your
        input (x), transformation (y), and expected outcome (w) to get your Z score.
      </DocP>

      <CodeBlock
        lang="bash"
        code={`blackroad z-check \\
...
function DefaultContent({ id }) {
  ...
  return (
    <div>
      ...
      <DocP>
        Documentation for this section is being written. Check back soon or contribute on GitHub.
      </DocP>
      <Callout type="info">
        This page is a placeholder. The BlackRoad documentation is under active development
        alongside the OS itself.
      </Callout>
      <CodeBlock
        lang="bash"
